# Auth Module — API Reference & Frontend Guide

This document is the complete frontend contract for admin authentication: password login, mandatory email OTP, token rotation, session inspection, logout, password recovery/change, and email two-factor status. It covers both the request/response API reference and the frontend integration guidance.
...
- CORS defaults to `http://localhost:5173` and `http://localhost:3000`. Add the deployed frontend origin to `AUTH_ALLOWED_ORIGINS`. Do not deploy production with `*`.
...
Use an explicit auth state rather than several unrelated booleans. Keep the OTP challenge in component or session state together with its `purpose`. Do not reuse a challenge ID across `LOGIN_2FA` and `PASSWORD_RESET`.
...
10. Cancel or ignore stale UI results after login/logout/reset transitions so an older response cannot overwrite newer auth state.
...
- This API returns tokens in JSON; it does not provide `HttpOnly` cookie storage. If the admin panel must survive a full browser restart, use the product's approved secure persistence strategy.
...
There is intentionally no server or admin setting that can disable the login second factor. Only the newest login OTP is valid. A new OTP invalidates the previous pending OTP.
...
2. If `error.code` is `invalid_otp`, keep the same challenge and show attempts left; the server remains authoritative.
...
Refresh, password reset, and the directly issued session return the same payload shape:
...
The previous pending login OTP is invalidated. A request before the cooldown returns `429 otp_resend_cooldown` with `Retry-After`; a request after the per-admin OTP window is exhausted returns `429 otp_too_many_requests`.
...
Use this endpoint on admin-panel startup when a token exists, and after refresh or password change. The safe admin projection never contains the password hash. `two_factor` is a fixed description of the mandatory policy, not a stored setting.
...
A successful `200 OK` changes the password, revokes all existing sessions, and returns a fresh authenticated session using the same session payload shape as login/2FA verification: `two_factor_required: false`, `tokens`, and `admin`. Replace any existing frontend token pair atomically and route to the admin panel.
...
If the request fails, keep the existing authenticated state unless the error indicates the token itself is no longer valid.
...
Email OTP is mandatory and cannot be disabled. There is no stored 2FA configuration and no enable, disable, confirm, or repair endpoint. The endpoint below is a fixed description of the policy, useful for showing the admin where the OTP is being sent.
...
- `method` has the value `EMAIL_OTP`.
...
|---:|---|---|
| `400` | `empty_body` | Required JSON body is missing. Fix the request. |
...
| `401` | `invalid_otp` | Stay on the OTP screen; keep a still-valid challenge and show attempts left. |
| `401` | `invalid_refresh_token` | Refresh token is missing, unknown, or expired. Clear auth state and route to login. |
| `401` | `refresh_token_reused` | Rotated token was replayed. All sessions were revoked; clear auth state and force login. |
...
| `403` | `account_disabled` | Admin account is inactive. Block login, clear session, and do not retry automatically. |
| `410` | `challenge_expired` | OTP challenge expired. Request/resend a code and restart verification. |
| `410` | `challenge_already_used` | Challenge was consumed or invalidated. Discard challenge and restart the flow. |
| `429` | `too_many_attempts` | Login limiter is active. Disable the action and honor `Retry-After`. |
...
| `502` | `email_delivery_failed` | Login/resend OTP email could not be delivered; ask the user to retry. Forgot-password intentionally masks this as its generic `202` response. |
